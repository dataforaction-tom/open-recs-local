...
import { MagicLinkInput, type MagicLinkInputT } from '@/lib/validation/auth';
...
export function MagicLinkForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<MagicLinkInputT>({
    resolver: zodResolver(MagicLinkInput),
    defaultValues: { email: '' },
  });

  function onSubmit(values: MagicLinkInputT) {
    startTransition(async () => {
      ...
      if (result.error) {
        setServerError(result.error.message ?? 'Could not send link. Try again.');
        return;
      }
      setSent(true);
    });
