...
- **Worker smoke test is flaky** — `tests/worker.smoke.test.ts` times out waiting for `[worker] ready` on stdout. 561/562 tests pass. Likely a startup race or Testcontainers timing issue on macOS. Must fix before 1.0.
- **Vitest is pinned to `^3.1.x`** (not 4.x). Vitest 4 ships a pre-release `rolldown` whose native binding fails under pnpm. The pinning applies as a group: `vitest@^3.1.0`, `@vitejs/plugin-react@^4`, `vite-tsconfig-paths@^5`, `vite@^6`. Do **not** bump piecemeal.
...
- macOS dev environment (was previously Windows — Docker Desktop auto-starts on macOS, no manual launch needed).
...
- **Don't install native-binding test tooling with `@latest`.** Pin the known-good combo above. See MISTAKES.md for the trail.
