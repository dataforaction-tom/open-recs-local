...
stateDiagram-v2
    ...
    Phase4 --> PhaseN: UI shell
    PhaseN --> Live: 1.0 release

    note right of Phase4: ← WE ARE HERE
```
...
- **Phase 3** ✅ implementation done (PR pending) — `/api/search` (hybrid RRF), `/api/keyword-search` (keyword + degrade path), `POST /api/chat-search` (streaming RAG over `source_pages` with `[[source:slug#page:N]]` citations), 60s query embedding cache, citation marker grammar.
- **Phase 4** 🔧 starting — UI shell (nav, DecisionFlow landing, dark mode, feature gates).
...
## Carry-overs flagged for Phase 4+
